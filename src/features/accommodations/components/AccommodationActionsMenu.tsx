...
import styles from './AccommodationActionsMenu.module.css'

type AccommodationActionsMenuProps = {
...
export function AccommodationActionsMenu({
  accommodation,
  disabled = false,
  onEdit,
  onChangeStatus,
  onDelete,
}: AccommodationActionsMenuProps) {
  ...
  const changeStatus = (status: TripContentStatus) => {
    closeAndRun(() => onChangeStatus(accommodation, status))
  }
  ...
  return (
    ...
    >
      <button
        ...
        type="button"
        aria-label={`Acciones de ${accommodation.name}`}
        aria-haspopup="menu"
      ...
      {isOpen && (
        ...
        >
          <button
            type="button"
            role="menuitem"
            onClick={() => closeAndRun(() => onEdit(accommodation))}
          >
            Editar
          </button>
              ...
              {showStatusActions && (
                <div className={styles.statusActions} role="group">
                  {statusActions
                    .filter(
                      ({ status }) =>
                        status !== accommodation.contentStatus,
                    )
                    .map(({ status, label }) => (
                      <button
                        key={status}
          ...
          <button
            ...
            type="button"
            role="menuitem"
            onClick={() => closeAndRun(() => onDelete(accommodation))}
          >
            Eliminar
          </button>
        </div>
