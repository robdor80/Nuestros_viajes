...
type BudgetFormModalProps = {
  ...
  onCancel: () => void
  onSave: (data: SaveBudgetData) => Promise<void>
}
...
export function BudgetFormModal({
  ...
  const [values, setValues] = useState<BudgetFormData>(() =>
    budget ? budgetToFormData(budget) : emptyBudgetFormData,
  )
  ...
  const calculations = useMemo(
    () => calculateBudget(values, automaticCosts),
    [automaticCosts, values],
  )
  const automaticRows = [
    automaticCosts.accommodationTotal > 0 && {
      help: 'Calculado desde los alojamientos del viaje.',
      label: 'Hotel',
      value: formatBudgetAmount(automaticCosts.accommodationTotal),
    },
    automaticCosts.parkingTotal > 0 && {
      help: 'Calculado desde el parking de pago en Alojamiento.',
      label: 'Parking',
      value: formatBudgetAmount(automaticCosts.parkingTotal),
    },
    automaticCosts.ticketsTotal > 0 && {
      help: `Calculado desde Qué se verá para ${automaticCosts.participantCount} persona${
        automaticCosts.participantCount === 1 ? '' : 's'
      }.`,
      label: 'Entradas',
      value: formatBudgetAmount(automaticCosts.ticketsTotal),
    },
  ].filter(
    (row): row is { help: string; label: string; value: string } =>
      Boolean(row),
  )
  ...
  const updateField = <T extends keyof BudgetFormData>(
    field: T,
    value: BudgetFormData[T],
  ) => {
  ...
  const save = async (contentStatus: TripContentStatus) => {
    ...
    const nextErrors = validateBudget(values, contentStatus, automaticCosts)
    ...
    try {
      await onSave({ ...values, contentStatus })
      onCancel()
    } catch (error) {
      setSubmissionError(
        ...
        <header className={styles.header}>
          <div>
            ...
            <h2 id="budget-form-title">
              {budget ? 'Editar presupuesto' : 'Añadir presupuesto'}
            </h2>
            <p>
              Los importes automáticos proceden de Alojamiento y Qué se verá.
            </p>
          </div>
          <button
        ...
        >
          ...
          <div className={styles.fields}>
            {automaticRows.map((row) => (
              <div key={row.label} className={styles.hotel}>
                <span>{row.label}</span>
            ...
            <fieldset className={styles.section}>
              <legend>Conceptos previstos</legend>
              {amountFields.map(({ field, label }, index) => (
            ...
            <fieldset className={styles.section}>
              <legend>Límite e imprevistos</legend>
              ...
              <label className={styles.switchField}>
                <span>Añadir colchón para imprevistos</span>
                <input
                  type="checkbox"
                  role="switch"
                  checked={values.contingencyEnabled}
                  disabled={isSaving}
                  onChange={(event) =>
                    updateField('contingencyEnabled', event.target.checked)
                  }
                />
              </label>
              {values.contingencyEnabled && (
                <label className={styles.field}>
                  <span>Porcentaje de imprevistos</span>
                  <select
                    ...
                    onChange={(event) =>
                      updateField(
                        ...
                        Number(
                          event.target.value,
                        ) as BudgetFormData['contingencyPercentage'],
                      )
                    }
                  >
                    {contingencyPercentages.map((percentage) => (
                ...
                <div className={styles.calculated}>
                  ...
                  <strong>
                    {calculations.contingency > 0
                      ? formatBudgetAmount(calculations.contingency)
                      : '—'}
                  </strong>
                </div>
              )}
            ...
            <div className={styles.summary} aria-live="polite">
              ...
              <strong>
                {calculations.total > 0
                  ? formatBudgetAmount(calculations.total)
                  : '—'}
              </strong>
              {calculations.remainingMargin !== null && (
                <>
                  ...
                  <strong
                    className={
                      calculations.remainingMargin < 0 ? styles.negative : ''
                    }
                  >
